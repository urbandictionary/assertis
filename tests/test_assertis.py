...
def test_empty():
    exit_code, report = generate_report(Path("cases/empty"))
    assert exit_code == 0
...
def test_added():
    exit_code, report = generate_report(Path("cases/files_added"))
    assert exit_code == 1
...
def test_changed():
    exit_code, report = generate_report(Path("cases/files_changed"))
    assert exit_code == 1
    assert report.has_changes is True
...
def test_changed_lot():
    exit_code, report = generate_report(Path("cases/files_changed_lot"))
    assert exit_code == 1
    assert report.has_changes is True
    assert {k: v for k, v in report.summary.items() if v != 0} == {"changed": 1}
...
def test_changed_mode():
    exit_code, report = generate_report(Path("cases/files_changed_mode"))
    assert exit_code == 1
    assert report.has_changes is True
    assert {k: v for k, v in report.summary.items() if v != 0} == {"changed": 1}
...
def test_changed_size():
    exit_code, report = generate_report(Path("cases/files_changed_size"))
    assert exit_code == 1
    assert report.has_changes is True
    assert {k: v for k, v in report.summary.items() if v != 0} == {"changed": 1}
...
def test_removed():
    exit_code, report = generate_report(Path("cases/files_removed"))
    assert exit_code == 1
    assert report.has_changes is True
...
def test_unchanged():
    exit_code, report = generate_report(Path("cases/files_unchanged"))
    assert exit_code == 0
    assert report.has_changes is False
